...
<body class="min-h-screen bg-[#0d0d14] flex items-center justify-center px-4"
    ...
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div
            class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-indigo-600/5 rounded-full blur-3xl">
        </div>
    </div>

    {{-- Tarjeta principal con Alpine.js --}}
    <div x-data="{ cargado: false, hovering: false, mostrarPass: false }" x-init="setTimeout(() => cargado = true, 100)" x-show="cargado"
        x-transition:enter="transition ease-out duration-700"
        x-transition:enter-start="opacity-0 translate-y-8 scale-95"
        x-transition:enter-end="opacity-100 translate-y-0 scale-100" class="relative w-full max-w-md">
        <div class="bg-[#13131f]/90 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl shadow-black/50">
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                <div>
                    <label for="email"
                        class="block text-white/60 text-xs font-medium mb-2 uppercase tracking-widest">
                        Correo electrónico
                    </label>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input id="password" :type="mostrarPass ? 'text' : 'password'" name="password"
                        ...
                        <button type="button" @click="mostrarPass = !mostrarPass"
                            ...
                            <svg x-show="!mostrarPass" class="w-4 h-4" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <button type="submit" @mouseenter="hovering = true" @mouseleave="hovering = false"
                    ...
                    :style="hovering ?
                        'background: linear-gradient(135deg, #4f46e5, #7c3aed); box-shadow: 0 0 30px rgba(99,102,241,0.4)' :
